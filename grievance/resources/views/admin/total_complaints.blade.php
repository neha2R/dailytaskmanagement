...
<div class="row" style="margin-bottom: 10px;">
        ...
        <div class="dropdown-primary dropdown open">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdown-2" data-dropdown-in="fadeIn"
                ...
                <a class="dropdown-item waves-light waves-effect" href="{{ request()->fullUrlWithQuery(['type' => 'crossedtl'])}}">Complaints with Crossed
                    ...
                    <a class="dropdown-item waves-light waves-effect" href="{{route('totalcomplaints')}}">All Complaints</a>
            </div>
        </div>
    </div>
    <div class="col-md-3">
    ...
    <div class="col-md-4" style="margin-top:20px;">
        <form method="GET" id="datefilter">
            ...
            <form id="date-filter"  action="{{route('totalcomplaints')}}">
...
@endphp
 data-img='<img src="{{\Storage::disk('public')->url($complaintattachment[$i]->media_name)}}" style="max-width: 300px;max-height: 200px;"/>'  data-Resolve="{{optional($Resolvedd)->resolution}}"
                                ...
                                <td>
                                 @php $Transfer=App\Models\Transition::where('complaintid',$item->complaint->id)->orderBy('id', 'desc')->latest()->first(); @endphp
                                ...
                                <th>
                                    <div class="animation-model">
                                        <button type="button"
                                            ...
                                            data-toggle="modal" data-target="#detailsmodal" data-details="{{$item1}}"
                                            @if ($item1->image)
                                   ...
                                   data-img="No Image"
                                    ...
                                    @endif
                                            ...
                                            Details
                                                                  ...
                                                                  <th>{{ App\User::find($item1->createdby)->email ?? 'N/A' }}</th>
                                 <td>
                                 @php $Transfer=App\Models\Transition::where('complaintid',$item1->id)->orderBy('id', 'desc')->latest()->first(); @endphp
                                       @if(!empty($Transfer->touserrelation->name))
